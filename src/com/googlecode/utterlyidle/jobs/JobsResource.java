...
public class JobsResource {
    ...
    public Map<String, Object> list() {
        return Maps.map("items", (Object) storage.jobs().
                sortBy(descending(created)).
                map(new Callable1<Job, Map<String, Object>>() {
                    @Override
                    public Map<String, Object> call(final Job job) throws Exception {
                        return jobModel(job);
                    }
                }));
    }
    ...
    public Response get(@PathParam("id") final UUID id) {
        return jobResponse(id, Exceptions.optional(new Callable1<Job, Response>() {
            @Override
        ...
        return new HashMap<String, Object>() {
            {
                ...
                putOptionally(this, "response", job.response().map(new Callable1<Response, Map<String, Object>>() {
                    @Override
                    public Map<String, Object> call(final Response response) throws Exception {
                        return ScheduleResource.asModel(response);
                    }
                }));
            }
        };
